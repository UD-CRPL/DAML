...
def train_model(dataset, labels, combination, tuning):
    ...
    if(combination == 0):
        ...
        if(tuning):
            ...
            randomized_search_1 = RandomizedSearchCV(model_1, rf_parameters, n_iter=30,
                        n_jobs=-1, verbose=0, cv=5,
                        scoring='roc_auc', refit=True, random_state=42)
            randomized_search_2 = RandomizedSearchCV(model_2, svm_parameters, n_iter=30,
                        n_jobs=-1, verbose=0, cv=5,
                        scoring='roc_auc', refit=True, random_state=42)
    # Model 1: Random Forest, Model 2: Gradient Boosting
        ...
        if(tuning):
            randomized_search_1 = RandomizedSearchCV(model_1, rf_parameters, n_iter=30,
                        n_jobs=-1, verbose=0, cv=5,
                        scoring='roc_auc', refit=True, random_state=42)
            randomized_search_2 =  RandomizedSearchCV(model_2, gdb_parameters, n_iter=30,
                        n_jobs=-1, verbose=0, cv=5,
                                scoring='roc_auc', refit=True, random_state=42)
    # Model 1: Gradient Boosting, Model 2: SVM
    elif(combination == 2):
        # Initializes Gradient Boosting model
        model_1 = xgboost.XGBClassifier(silence=1)
        # initializes SVM
        model_2 = SVC()
        # Initializes the hyperparameter tuning (Random Search) for each model
        if(tuning):
            randomized_search_1 =  RandomizedSearchCV(model_1, gdb_parameters, n_iter=30,
                        n_jobs=-1, verbose=0, cv=5,
                        scoring='roc_auc', refit=True, random_state=42)
            randomized_search_2 =  RandomizedSearchCV(model_2, svm_parameters, n_iter=30,
                        n_jobs=-1, verbose=0, cv=5,
